...
export default async function OwnerDashboardPage() {
  ...
  const { data: listings, error } = await supabase
    .from("listings")
    .select(
      "id,resort_name,city,check_in_date,check_out_date,unit_type,owner_price_cents,normal_price_cents,is_active,created_at"
    )
    .eq("owner_id", user.id)
  ...
  if (listingIds.length > 0) {
    const { data: offers } = await supabase.from("offers").select("listing_id").in("listing_id", listingIds);
    ...
    <main className="mx-auto max-w-5xl p-8">
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <Link className="rounded bg-zinc-900 px-3 py-2 text-sm text-white" href="/listings/new">
            Add listing
          </Link>
          <SignOutButton />
        </div>
