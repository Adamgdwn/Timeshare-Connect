...
type LeaveResortReviewFormProps = {
  bookingId: string;
  listingId: string;
  resortLabel: string;
};

export default function LeaveResortReviewForm({ bookingId, listingId, resortLabel }: LeaveResortReviewFormProps) {
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setMessage("Resort review submitted.");
      setComment("");
      router.refresh();
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Failed to submit resort review.");
    } finally {
  ...
  return (
    <form className="space-y-2 rounded border border-zinc-200 p-3" onSubmit={onSubmit}>
      <h3 className="text-sm font-semibold">Rate Resort</h3>
      <p className="text-xs text-zinc-600">{resortLabel}</p>
      ...
      <textarea
        ...
        placeholder="Optional resort-specific comment"
        value={comment}
      ...
      <button
        ...
        type="submit"
      >
        {isSubmitting ? "Submitting..." : "Submit resort review"}
      </button>
